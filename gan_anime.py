...
def discriminative_network(x):
	...
	act=None
	seq=[
		convolutional.Conv2D(32,(4,4),strides=(1, 1), padding='same',activation=act),
		advanced_activations.LeakyReLU(alpha=0.3),
		convolutional.Conv2D(32,(4,4),strides=(2, 2), padding='same',activation=act),
		#normalization.BatchNormalization(),
		advanced_activations.LeakyReLU(alpha=0.3),
		convolutional.Conv2D(64,(4,4),strides=(1, 1), padding='same',activation=act),
		advanced_activations.LeakyReLU(alpha=0.3),
...
for epoch in range(nepoch):
	...
	if np.isnan(loss_g) or np.isnan(loss_d):
		print("[ERR0R]")
